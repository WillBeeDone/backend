...
public class OfferServiceImpl implements OfferService {
    ...
    public List<OfferFilterResponseDto> getFilteredOffers(String cityName, String category, String keyPhrase) {
        return repository.findAll()
                ...
                .filter(offer -> "all".equals(cityName) || offer.getUser().getLocation().getCityName().equals(cityName))
                .filter(offer -> "all".equals(category) || offer.getCategory().equals(category))
                .filter(offer -> "all".equals(keyPhrase) ||
                        offer.getUser().getFirstName().contains(keyPhrase) ||
                        offer.getUser().getLastName().contains(keyPhrase) ||
                        offer.getTitle().contains(keyPhrase) ||
                        offer.getDescription().contains(keyPhrase))
                .map(mappingService::mapEntityToFilterResponseDto)
                .toList();
    }
